...
function add() {
    location.href = "WildlifeAdd.html?pages=" + nowpages + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, ""));
}
...
function tablelist(WebAPIPath) {
    $.ajax({
        url: WebAPIPath + "/api/wildlifemanager/list",
        async: false,
        type: "GET",
            ...
            $.ajax({
                ...
                success: function (data) {                   
                    ...
                    for (var i = 0; i < list.length; i++) {
                        var wildcate = list[i].JQ_WildlifeCategoryID == null ? "" : list[i].JQ_WildlifeCategoryID;
                        ...
                        "<tr>" +
                            ...
                            "<td style='text-align:center;'>" + wildcate + "</td>" +
                            "<td style='text-align:center;'>" + list[i].OrderID + "</td>" +
                        "<td style='text-align:center;' name='edit'><a href='WildlifeEdit.html?ID=" + list[i].ID + "&pages=" + nowpages + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "'>编辑</a>" +
                        "&nbsp;<a href='WildlifeDetailManager.html?ID=" + list[i].ID + "&pages=" + nowpages + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "'>详细信息</a>" +
                        "</td>" +
                        "</tr>";
                    }
                    if (msghtml == "") {
                    ...
                    crossDomainAjax(geturl() + "/api/permission/navshowlist?NavCode=wildlifemanager&UserID=" + CMSUserID, PerSuccessCallback, ErrorCallback);
                    layui.use('form', function () {
...
function del(WebAPIPath) {
    ...
    layer.confirm("你确定要删除吗？", { icon: 3, title: '提示' }, function (index) {
        $.ajax({
            url: WebAPIPath + "/api/wildlifemanager/delete",
            async: false,
            type: "POST",
            headers: { Authorization: GetCMSData().CMSToken },
