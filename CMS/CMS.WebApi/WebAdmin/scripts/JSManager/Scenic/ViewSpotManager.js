...
function add() {
    location.href = "AddViewSpot.html?pages=" + nowpages + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, ""));
}
list();
$("#cx").click(function () {
    loadpages = 1;
...
function del() {
    ...
    layer.confirm("你确定要删除选中项？", { icon: 3, title: '提示' }, function (index) {
        PostAjax('/api/viewspot/delete', "IDs=" + ids.substring(0, ids.length - 1) + "&UserID=" + CMSUserID, delSuccessCallback, ErrorCallback);
    });
}
function delSuccessCallback(data, index) {
    if (data.Success == "1") {
        ...
        if (data.Result == "景点详情信息的不能删除") {
            layer.msg("无法删除，存在详细信息或推荐路线！");
            return false;
        } else {
...
function list() {
    layer.load(2);
    crossDomainAjax("/api/viewspot/list?Pageindex=1&Keywords=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "&ran=" + Math.random(), SuccessCallback, ErrorCallback);
    setTimeout(function () {
        ...
        laypage({
            ...
            jump: function (obj) {
                nowpages = obj.curr;
                crossDomainAjax("/api/viewspot/list?Pageindex=" + obj.curr + "&Keywords=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "&ran=" + Math.random(), PageSuccessCallback, ErrorCallback);
            }
        });
    }, 500);
}
...
function PageSuccessCallback(data, index) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        "<td style='text-align:center;' ><a href='EditViewSpot.html?ID=" + list[i].ID + "&pages=" + nowpages + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "' name='edit'>编辑</a>&nbsp;&nbsp;<a href='ViewSpotDetailManager.html?ViewSpotID=" + list[i].ID + "&pages=" + nowpages + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "'>详细信息</a></td>" +
        "</tr>";
    }
    if (msghtml == "") {
    ...
    crossDomainAjax("/api/permission/navshowlist?NavCode=ViewSpot&UserID=" + CMSUserID + "&ran=" + Math.random(), PerSuccessCallback, ErrorCallback);
