...
var uploader;
var attachImage;
//alert(GetQueryString("pages") + ',' + GetQueryString("parent"))
function backs() {
    parent.location.href = "ColumnManager.aspx?pages=" + GetQueryString("pages") + "&parent=" + GetQueryString("parent") + "&keyword=" + GetQueryString("keyword");
}
...
angularjs.controller('columnbodyyz', function ($scope) {
    $scope.isDup = function (name) {
        //验证栏目名称是否重复
        //crossDomainAjax(geturl() + "/api/category/valid?ID=&Name=" + name, nameSuccessCallback, ErrorCallback);
        $.ajax({
            url: apiurl + "/api/category/valid",
    ...
    $scope.isDupNo = function (no) {
        //验证栏目标识码是否重复
        //crossDomainAjax(geturl() + "/api/category/novalid?ID=&RefNo=" + no, isDupNoSuccessCallback, ErrorCallback);
        $.ajax({
            url: apiurl + "/api/category/novalid",
            headers: { Authorization: GetCMSData().CMSToken },
            type: "GET",
            async: false,
            data: "ID=&RefNo=" + no,
...
$("#submit").click(function () {
    ...
    PostAjaxJson(geturl() + '/api/category/addInfo', JSON.stringify(postdata), AddSuccessCallback, ErrorCallback);
    //$.ajax({
    //    url: geturl() + "/api/category/addInfo",
    //    type: "POST",
    ...
    //        //alert(data.Success)
    //        if (data.Success == "1") {
    //            layer.msg("新增栏目成功！", { time: 1000 }, function () { parent.location.href = "ColumnManager.aspx"; });
    //        } else {
    //            layer.msg("新增栏目失败！", { time: 1000 });
...
function AddSuccessCallback(data, index) {
    if (data.Success == "1") {
        layer.msg("新增栏目成功！", { time: 1000 }, function () { backs(); });
    } else {
        layer.msg("新增栏目失败！", { time: 1000 });
        $("#submit").removeClass("clicking");
    }
