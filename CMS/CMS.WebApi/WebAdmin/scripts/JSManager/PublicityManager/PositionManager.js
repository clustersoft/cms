...
function add() {
    location.href = "AddPositionInfo.aspx?pages=" + nowpages + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "&PublicityTypesID=" + $("#PublicityTypesID").val();
}
crossDomainAjax("/api/publicity/positiontypelist?ran=" + Math.random(), TypeSuccessCallback, ErrorCallback);
list();
$("#cx").click(function () {
    loadpages = 1;
    list();
...
function del() {
    ...
    layer.confirm("你确定要删除选中项？", { icon: 3, title: '提示' }, function (index) {
        PostAjax('/api/publicity/deleteposition', "IDs=" + ids.substring(0, ids.length - 1) + "&UserID=" + CMSUserID, delSuccessCallback, ErrorCallback);
    });
}
function delSuccessCallback(data, index) {
    if (data.Success == "1") {
        ...
        list();
    } else if (data.Success == "0" && data.Result == "位置下有链接不可删除") {
        layer.msg("位置信息正在使用无法删除！");
        return false;
    } else {
...
function list() {
    crossDomainAjax("/api/publicity/positionlist?pageIndex=1&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "&PublicityTypesID=" + $("#PublicityTypesID").val() + "&ran=" + Math.random(), SuccessCallback, ErrorCallback);
    ...
    laypage({
        ...
        jump: function (obj) {
            nowpages = obj.curr;
            crossDomainAjax("/api/publicity/positionlist?pageIndex=" + obj.curr + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "&PublicityTypesID=" + $("#PublicityTypesID").val() + "&ran=" + Math.random(), PageSuccessCallback, ErrorCallback);
        }
    });
}
function TypeSuccessCallback(data, index) {
    var list = eval(data.Result);
    for (var i = 0; i < list.length; i++) {
        if (list[i].ID == GetQueryString("PublicityTypesID")) {
            $("#PublicityTypesID").append("<option value='" + list[i].ID + "' selected>" + list[i].TypeName + "</option>");
        } else {
...
function PageSuccessCallback(data, index) {
    ...
    "<th style='text-align: center;'>位置类别</th>" +
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        "<tr>" +
            ...
            "<td style='text-align:center;'>" + list[i].PublicityTypesName + "</td>" +
            "<td style='text-align:center;'>" + list[i].OrderID + "</td>" +
            "<td style='text-align:center;' name='edit'><a href='EditPositionInfo.aspx?ID=" + list[i].ID + "&pages=" + nowpages + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "&PublicityTypesID=" + $("#PublicityTypesID").val() + "'>编辑</a></td>" +
        "</tr>";
    }
    if (msghtml == "") {
        msghtml = "<td colspan=\"6\" style=\"text-align: center;\">暂无数据</td>";
    }
    ...
    crossDomainAjax("/api/permission/navshowlist?NavCode=Position&UserID=" + CMSUserID + "&ran=" + Math.random(), PerSuccessCallback, ErrorCallback);
