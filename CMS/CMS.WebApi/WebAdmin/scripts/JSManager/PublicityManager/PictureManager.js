...
function add() {
    location.href = "AddPicLinkInfo.aspx?pages=" + nowpages + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "&PublicityCateID=" + $("#PublicityTypesID").val();
}
        crossDomainAjax( "/api/publicity/selpositionlist?PublicityTypesID=1&ran=" + Math.random(), CateSuccessCallback, ErrorCallback);
        list();
        $("#cx").click(function () {
            loadpages = 1;
            list();
        ...
        function list() {
            crossDomainAjax("/api/publicity/contentlist?pageIndex=1&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "&PublicityTypesID=1&PublicityCateID=" + $("#PublicityTypesID").val() + "&ran=" + Math.random(), SuccessCallback, ErrorCallback);
            ...
            laypage({
                ...
                jump: function (obj) {
                    nowpages = obj.curr;
                    crossDomainAjax("/api/publicity/contentlist?pageIndex=" + obj.curr + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "&PublicityTypesID=1&PublicityCateID=" + $("#PublicityTypesID").val() + "&ran=" + Math.random(), PageSuccessCallback, ErrorCallback);
                }
            });
        }
        ...
        function PageSuccessCallback(data, index) {
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                "<tr>" +
                    ...
                    "<td style='text-align:center;' name='edit'><a href='EditPicLinkInfo.aspx?ID=" + list[i].ID + "&pages=" + nowpages + "&keyword=" + escape($("#skey").val().replace(/(^\s*)|(\s*$)/g, "")) + "&PublicityCateID=" + $("#PublicityTypesID").val() + "'>编辑</a></td>" +
                "</tr>";
            }
            if (msghtml == "") {
            ...
            crossDomainAjax("/api/permission/navshowlist?NavCode=PicLink&UserID=" + CMSUserID + "&ran=" + Math.random(), PerSuccessCallback, ErrorCallback);
