...
layui.define(['element', 'common'], function (exports) {
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            ...
            if (data.title == "后台首页") {
                ...
                $('.admin-nav-card').find('div.layui-tab-content > div').eq(0).children('iframe')[0].contentWindow.location = data.href;
            }
            else {
                element.tabChange(ELEM.tabFilter, that.getTabId(data.title));
    ...
    //element.tabAdd(ELEM.tabFilter, {
    //    title: "后台首页",
    //    content: '<iframe src="HomePage.aspx" data-id="-99" lay-id="-99"></iframe>',
    //    id: new Date().getTime()
    //});
});